...
const publicSchema = z.object({
  ...
  // Business Studio is now a genuinely separate app (apps/studio), not a
  // route group in this one — surfaceOrigin("studio") only resolves once
  // NEXT_PUBLIC_ROOT_DOMAIN and real subdomains exist. Until then (all of
  // dev, and any deploy without subdomains yet), this is the only way this
  // app knows where to send a vendor to sign in.
  NEXT_PUBLIC_STUDIO_APP_URL: z.string().url().default("http://localhost:5173"),
});
...
const serverSchema = z.object({
  ...
  // Invoice PDF rendering (vendor "generate invoice" flow) - reuses Bali's
  // Gotenberg container (a generic, stateless HTML->PDF renderer), behind its
  // own public route + credential. Optional until a vendor actually invokes it.
...
export const publicEnv = publicSchema.parse({
  ...
  NEXT_PUBLIC_STUDIO_APP_URL: process.env.NEXT_PUBLIC_STUDIO_APP_URL,
});
