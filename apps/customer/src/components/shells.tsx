...
/**
 * Customer surface — a public marketplace site, not a logged-in app. Header
 * chrome mirrors HostelSure's (browse / log in / list-your-business), because
 * most traffic here browses anonymously before ever signing in: a persistent
 * app sidebar is the wrong shape for that, the way it would be for any
 * storefront. Vendor Studio and Admin stay sidebar apps — those really are
 * logged-in work surfaces.
 */
export function CustomerShell({
  ...
  const brand = (
    <Link href="/" className="flex flex-col leading-none">
      <span className="font-serif text-2xl font-semibold tracking-tight text-primary">Nexa</span>
      <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Powered by ERA Systems</span>
    </Link>
  );

  const mobileNav: NavItem[] = user
    ? [
        { label: "Browse Vendors", href: "/search", icon: Search },
  ...
  const mobileNavWithActive = mobileNav.map((item) => ({
    ...item,
  ...
  const mobileFooter = (
    <>
      <Link href={"/apply" as Route} onClick={() => setOpen(false)}>
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <header className="sticky top-0 z-40 border-b border-border bg-card/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 md:px-8">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            <Link href={"/apply" as Route} className="text-sm font-medium text-foreground transition hover:text-primary">
              Be a Vendor
            </Link>
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center gap-1.5 text-sm font-medium text-foreground outline-none transition hover:text-primary">
              ...
              <Link href={"/login" as Route} className="text-sm font-medium text-foreground transition hover:text-primary">
                Log in
              </Link>
            )}
          ...
          <button
            ...
            aria-label="Open menu"
            onClick={() => setOpen(true)}
            className="flex size-9 items-center justify-center rounded-md hover:bg-black/5 md:hidden"
          >
            <Menu className="size-5" />
          </button>
        </div>
      ...
      {/* Mobile drawer — same slide-in pattern as Studio/Admin, repurposed
          with public-site links instead of an app nav. */}
      <div
        aria-hidden
        onClick={() => setOpen(false)}
        className={cn(
      ...
      <div
        ...
        aria-label="Menu"
        className={cn(
          "fixed inset-y-0 right-0 z-50 flex w-72 max-w-[85vw] flex-col bg-card shadow-xl transition-transform duration-200 md:hidden",
          open ? "translate-x-0" : "translate-x-full",
        )}
      >
        ...
        <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3">
          <NavLinks items={mobileNavWithActive} onNavigate={() => setOpen(false)} />
        </nav>
        <div className="border-t border-border p-4">{mobileFooter}</div>
      </div>
...
export function AdminShell({
  ...
  // adminIcon() resolves client-side here — Server Components can pass plain
  // label/href data across the boundary, but not a reference to a function
  // that lives in this "use client" module.
