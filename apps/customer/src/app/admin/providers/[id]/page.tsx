...
import {
  approveProviderAndListingsAction,
...
export default async function ProviderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const categoryNames = (
    provider.provider_categories as unknown as Array<{ categories: { name: string } | null }> | null
  )
    ...
    <>
      ...
      <div className="mb-4 mt-4 flex flex-wrap items-center gap-2">
        {provider.status === "pending" ? (
          <>
            <ActionButton
              label={pendingListings.length > 0 ? `Approve vendor + ${pendingListings.length} listing${pendingListings.length === 1 ? "" : "s"}` : "Approve this vendor"}
              variant="primary"
              confirm={
              ...
              run={approveProviderAndListingsAction.bind(null, provider.id)}
            />
            <ActionButton
              label="Reject"
              variant="danger"
