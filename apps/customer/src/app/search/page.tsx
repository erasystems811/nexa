...
// Ported to api-server (Phase 1 of the monorepo split) — this now calls the
// shared API instead of talking to Supabase directly. See packages/api-client.
import { searchListings, searchVendors, listCategories, listCities, type ListingFilters } from "@nexa/api-client";
...
/**
 * Two jobs on one page, same as HostelSure's own hostel-first browsing:
 *
 *   Browsing — no typed query, maybe a category chip — shows VENDORS. One card
 *   per business, opening onto their menu, the way tapping a hostel opens its
 *   rooms. This is how a customer explores.
 *
 ...
 *
 * Location is free text either way — see modules/search's location filter.
 */
export default async function SearchPage({
  ...
  const [listings, vendors] = await Promise.all([
    ...
    isSearching
      ? Promise.resolve([])
      : searchVendors({ categorySlug: sp.category, location: sp.location }),
  ]);
  ...
  // Preserves every other active filter when a chip changes only the category.
  const chipHref = (categorySlug?: string): Route => {
    const next = new URLSearchParams();
    if (sp.q) next.set("q", sp.q);
    if (sp.location) next.set("location", sp.location);
    ...
    <div className="pb-6">
      ...
      <div className="sticky top-0 z-30 -mx-4 border-b bg-background/90 px-4 pb-3 pt-4 backdrop-blur-md md:-mx-8 md:px-8">
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          ...
          <LocationFilter cities={cities} defaultValue={sp.location ?? ""} />
        </div>

        <nav className="mt-4 -mx-4 flex gap-2 overflow-x-auto px-4 pb-1 md:-mx-8 md:px-8">
          <Chip href={chipHref()} label="All" active={!sp.category} />
          {categories.map((c) => (
            <Chip
              key={c.id}
      ...
      {count === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">
          {isSearching
            ? "No service matches that yet. Try a different word, a different location, or browse the categories above."
