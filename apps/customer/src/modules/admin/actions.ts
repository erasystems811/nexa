...
import type { BookingStatus, VendorTier } from "@/lib/db/types";
...
export async function approveProviderAndListingsAction(providerId: string): Promise<void> {
  const { warning } = await admin.approveProviderAndListings(await actor(P.providersApprove), providerId);
  revalidatePath("/admin/providers");
  revalidatePath(`/admin/providers/${providerId}`);
...
export async function setCategoryVendorTierAction(categoryId: string, tier: VendorTier): Promise<void> {
  await admin.setCategoryVendorTier(await actor(P.settingsManage), categoryId, tier);
  revalidatePath("/admin/categories");
  revalidatePath("/");
}
