...
import { acceptOfferAsAdmin, sendApprovedOfferAsAdmin } from "@/modules/bookings/offers";
import { checkout, cancelBookingByCustomer, acceptBooking, rejectBooking, BookingsError } from "@/modules/bookings";
import { sendInvoiceToCustomer } from "./invoice";
...
/**
 * A vendor asking to formalize an agreed price into a real, payable invoice -
 * the one way from a discussed number to something the customer can act on
 * (see the "generate invoice" block in handleIncomingWhatsappText). Captures
 * anything typed on the same line, e.g. "generate invoice cake - 150k".
 */
const GENERATE_INVOICE_TRIGGER = /^(?:generate|create|make)\s+invoice\b\s*(.*)$/i;
...
export async function handleIncomingWhatsappText(input: WhatsappTextMessage): Promise<void> {
  ...
  if (!conversationId || !resolvedSenderId || !resolvedSide) {
    ...
    // The customer's very first message through this link — say what's about
    // to happen and how to leave, since nothing else ever tells them. "menu"
    // already works as an EXIT_COMMAND below; this just announces it.
    if (bound.side === "customer" && bound.isNewBind) {
      await sendWhatsappText({
        to: input.waId,
        body:
  ...
  // A bare number from a vendor is now just chat - "generate invoice" is the
  // one path from a discussed price to something the customer can pay, and
  // it always goes through the vendor's own 1/0 approval first.
  if (resolvedSide === "vendor") {
    const draftHandled = await handleVendorInvoiceReply({ conversationId, waId: input.waId, text: input.text });
    if (draftHandled) return;

    if (GENERATE_INVOICE_TRIGGER.test(input.text.trim())) {
      await startInvoiceDraft({ conversationId, waId: input.waId, text: input.text });
      return;
    }
  }
...
/**
 * "<item> - <amount>" or a bare amount (keeping whatever item is already
 * known). Used both for text trailing the "generate invoice" trigger and for
 * the vendor's reply while editing an open draft.
 */
function parseItemAndAmount(text: string, fallbackItem: string): { item: string; amountKobo: number } | null {
  const trimmed = text.trim();
  if (!trimmed) return null;

  const dashSplit = trimmed.match(/^(.+?)\s*[-–—]\s*(.+)$/);
  if (dashSplit) {
    const amountKobo = parseVendorQuote(dashSplit[2]!);
    if (amountKobo !== null) return { item: dashSplit[1]!.trim() || fallbackItem, amountKobo };
  }

  const amountKobo = parseVendorQuote(trimmed);
  if (amountKobo !== null) return { item: fallbackItem, amountKobo };

  return null;
}

/**
 * Scans this vendor's own recent messages for the last one that read as
 * nothing but a price (see VENDOR_QUOTE) - the number "generate invoice" is
 * meant to formalize, when the vendor didn't type it on the same line.
 */
async function findRecentVendorQuote(conversationId: string): Promise<number | null> {
  const db = createAdminClient();
  const { data } = await db
    .from("messages")
    .select("body")
    .eq("conversation_id", conversationId)
    .eq("origin", "whatsapp_vendor")
    .order("created_at", { ascending: false })
    .limit(20);

  for (const row of data ?? []) {
    const amountKobo = parseVendorQuote(row.body);
    if (amountKobo !== null) return amountKobo;
  }
  return null;
}

function invoicePreviewText(item: string, amountKobo: number): string {
  const amountNaira = (amountKobo / 100).toLocaleString("en-NG");
  return (
    `Invoice preview:\nItem: ${item}\nPrice: ₦${amountNaira}\n\n` +
    `Reply 1 to approve and send this to the customer, or 0 to change the item/price.`
  );
}

/**
 * "generate invoice" (with or without a trailing item/price) - builds the
 * draft the vendor has to approve before anything reaches the customer.
 */
async function startInvoiceDraft(input: { conversationId: string; waId: string; text: string }): Promise<void> {
  const db = createAdminClient();
  ...
  if (!conversation?.listing_id) {
    await sendWhatsappText({ to: input.waId, body: "There's no listing on this chat to invoice against." });
    return;
  }

  const fallbackItem = (conversation.listings as unknown as { title: string } | null)?.title ?? "Service";
  const trailing = input.text.trim().match(GENERATE_INVOICE_TRIGGER)?.[1]?.trim() ?? "";

  let item = fallbackItem;
  let amountKobo: number | null = null;

  if (trailing) {
    const parsed = parseItemAndAmount(trailing, fallbackItem);
    if (parsed) {
      item = parsed.item;
  ...
  if (amountKobo === null) {
    amountKobo = await findRecentVendorQuote(input.conversationId);
  }

  if (amountKobo === null) {
    await sendWhatsappText({
      to: input.waId,
      body: 'What\'s the price? Reply with just the amount (e.g. 150000 or 150k), then say "generate invoice" again.',
    });
    return;
  }

  await db
    .from("whatsapp_invoice_drafts")
    .upsert({ conversation_id: input.conversationId, item, amount_kobo: amountKobo, status: "awaiting_confirmation" });

  await sendWhatsappText({ to: input.waId, body: invoicePreviewText(item, amountKobo) });
}

/**
 * The vendor's reply to an open draft: "1" approves and sends the real
 * invoice, "0" starts an edit, and while editing the next message is read as
 * the corrected item/price. Returns false when there's no open draft, so the
 * caller falls through to ordinary chat handling.
 */
async function handleVendorInvoiceReply(input: { conversationId: string; waId: string; text: string }): Promise<boolean> {
  const db = createAdminClient();

  const { data: draft } = await db
    .from("whatsapp_invoice_drafts")
    .select("item, amount_kobo, status")
  ...
  if (draft.status === "awaiting_confirmation") {
    if (trimmed === "1") {
      await approveInvoiceDraft(input.conversationId, input.waId, draft.item, draft.amount_kobo);
      return true;
    }
    if (trimmed === "0") {
      await db
        .from("whatsapp_invoice_drafts")
        .update({ status: "awaiting_edit" })
        .eq("conversation_id", input.conversationId);
      await sendWhatsappText({
        to: input.waId,
        body: 'Reply with the item and price, e.g. "Cake - 150000", or just a new amount to keep the same item.',
      });
      return true;
    }
    await sendWhatsappText({ to: input.waId, body: "Reply 1 to approve, or 0 to change the item/price." });
    return true;
  }

  // status === "awaiting_edit"
  const parsed = parseItemAndAmount(trimmed, draft.item);
  if (!parsed) {
    await sendWhatsappText({
      to: input.waId,
      body: 'I couldn\'t read that. Reply with the item and price, e.g. "Cake - 150000".',
    });
    return true;
  }
  ...
  await db
    .from("whatsapp_invoice_drafts")
    .update({ item: parsed.item, amount_kobo: parsed.amountKobo, status: "awaiting_confirmation" })
    .eq("conversation_id", input.conversationId);

  await sendWhatsappText({ to: input.waId, body: invoicePreviewText(parsed.item, parsed.amountKobo) });
  return true;
}

/**
 * The vendor approved: creates the offer already-accepted (see
 * sendApprovedOfferAsAdmin) and sends the real invoice PDF straight to the
 * customer - no separate "Accept" tap. The payment link comes once the
 * customer replies with a date/time; findAcceptedOfferAwaitingCheckout
 * already handles that, unchanged.
 */
async function approveInvoiceDraft(
  conversationId: string,
  vendorWaId: string,
  item: string,
  amountKobo: number,
  ...
  await db.from("whatsapp_invoice_drafts").delete().eq("conversation_id", conversationId);
  ...
  if (!conversation?.listing_id) return;

  const context = await getWhatsappThreadContext(conversationId);
  if (!context?.customerWaId) {
    await sendWhatsappText({
      to: vendorWaId,
      body: "I couldn't find the customer's WhatsApp number to send the invoice to - they may not have messaged in yet.",
    });
    return;
  }

  const { offerId } = await sendApprovedOfferAsAdmin({
    conversationId,
  ...
  try {
    await sendInvoiceToCustomer({
      ...
      reference: offerId.slice(0, 8),
      caption:
        `${context.vendorName} sent you an invoice for "${item}". ` +
        `Reply here with your event date and time (e.g. ${SCHEDULE_EXAMPLE}) to get your payment link.`,
    });
  } catch (error) {
    await sendWhatsappText({
      to: vendorWaId,
      body: `Sorry, the invoice couldn't be sent: ${error instanceof Error ? error.message : "please try again"}`,
    });
    return;
  }
  ...
  await sendWhatsappText({
    to: vendorWaId,
    body: `Invoice sent to the customer for ₦${(amountKobo / 100).toLocaleString("en-NG")}.`,
  });
}
