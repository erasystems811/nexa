...
/**
 * The invoice-approval path (see handleIncomingWhatsappText's "generate
 * invoice" flow): the vendor has already reviewed the item/price and
 * approved it before anything reached the customer, so this creates the
 * offer already accepted - there is no separate customer "Accept" tap. The
 * invoice PDF the customer receives IS the offer; findAcceptedOfferAwaitingCheckout
 * picks it up from here exactly as it would an accepted button-tap offer.
 */
export async function sendApprovedOfferAsAdmin(input: {
  conversationId: string;
  listingId: string;
  providerId: string;
  customerId: string;
