...
export interface ListingFilters {
   ...
   * Matches against a provider's address, falling back to their city name so
   * today's city-only data (no address on file yet) still matches.
   */
  location?: string;
  minPriceKobo?: number;
  maxPriceKobo?: number;
...
/**
 * Free-text location resolves to a set of provider ids up front, rather than
 * an .eq on a fixed city slug — it has to match a specific area in
 * providers.address, not just the city join, and PostgREST can't OR across
 * two different embedded-table levels (providers.address vs
 * providers.cities.name) in a single filter. Two plain queries merged in JS,
 * not a single .or() with the term spliced into the filter string — that
 * string is parsed by PostgREST itself, so a location containing a comma or
 * parenthesis could otherwise reshape the filter instead of just being
 * searched for.
 *
...
export async function searchVendors(filters: {
  categorySlug?: string;
  /** Free text — see resolveLocationProviderIds. */
  location?: string;
  limit?: number;
