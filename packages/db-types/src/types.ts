...
export type VendorTier = Enum<"vendor_tier">;
...
/**
 * Only self_serve changes runtime behavior — no booking, no escrow, just a
 * WhatsApp introduction. paid_service and paid_performer both go through the
 * ordinary escrow flow; the split between them exists for labelling only.
 */
export const VENDOR_TIERS = [
  "self_serve",
  "paid_service",
  "paid_performer",
] as const satisfies readonly VendorTier[];

export const VENDOR_TIER_LABELS: Record<VendorTier, string> = {
  self_serve: "Self-serve vendor (not paid through Nexa)",
  paid_service: "Paid service/product provider",
