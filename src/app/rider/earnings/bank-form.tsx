...
import { saveRiderBankAction, type RiderFormState } from "@/modules/rider/actions";
import { Alert, Button, Field } from "@/components/ui";

export function RiderBankForm({
  defaults,
}: {
  defaults: { bank_code: string; bank_account_number: string; bank_account_name: string };
}) {
  const [state, action, pending] = useActionState(saveRiderBankAction, {} as RiderFormState);
