...
export interface BookingMoney {
  ...
  refundedKobo: number;
  /** What Nexa is still holding — the most that can be paid out or refunded. */
...
export function bookingMoney(
  ...
  return {
    customerPaidKobo: payment.held_kobo > 0 ? payment.held_kobo : payment.amount_kobo,
...
export async function releaseToVendor(
  ...
  // The one hard rule: you cannot pay out money you are not holding.
  const held = stillHeld(payment);
  if (amountKobo > held) {
    throw new AdminError(
      `Nexa is only holding ${formatKobo(held)} on this booking, so that is the most you can pay out.`,
    );
  }
  ...
  await audit(
    ...
    bookingId,
    { stillHeldKobo: held },
    { paidToVendorKobo: amountKobo, stillHeldAfterKobo: held - amountKobo },
  );
}
