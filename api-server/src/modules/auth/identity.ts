/** Which frontend an account belongs to. Mirrors apps/customer's Surface type. */
export type AuthApp = "customer" | "studio" | "admin";
 ...
 */
export function authEmailFor(app: AuthApp | null | undefined, realEmail: string): string {
  const email = realEmail.trim().toLowerCase();
  if (app === "studio") return tagEmail(email, "vendor");
  return email;
}
