...
import { sendPasswordResetCode, sendPasswordSetupCode } from "../email/resend.js";
...
function resetUrl(appUrl: string, email: string): string {
  return `${appUrl}/reset?email=${encodeURIComponent(email)}&step=code`;
}
...
export async function sendPasswordCode(input: {
  ...
  name?: string;
  purpose: "reset" | "setup";
  /** Which app's /reset page the link points to — customer, studio, or admin. */
  appUrl: string;
}): Promise<void> {
  const admin = createAdminClient();
  const email = input.email.trim().toLowerCase();
  ...
  const { data, error } = await admin.auth.admin.generateLink({
    type: "recovery",
    email: authEmail,
    options: { redirectTo: `${input.appUrl}/reset` },
  });
  if (error) throw new Error(error.message);
  ...
  const payload = { to: email, name: input.name, code, actionUrl: resetUrl(input.appUrl, email) };
 ...
 */
export async function trySendPasswordSetupCode(input: {
  email: string;
  authEmail?: string;
  name?: string;
  /** Which app's /reset page the link points to — studio for vendors, admin for staff. */
  appUrl: string;
  ...
  try {
    await sendPasswordCode({ email: input.email, authEmail: input.authEmail, name: input.name, purpose: "setup", appUrl: input.appUrl });
    return undefined;
  } catch {
