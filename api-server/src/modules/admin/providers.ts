...
import { authEmailFor } from "../auth/identity.js";
import { idTypeLabel, isIdentityVerified, REQUIRED_ID_COUNT } from "../provider/index.js";
import { adminDb, audit, AdminError } from "./context.js";
import { sendVerificationChangesRequested } from "../email/resend.js";
import { env } from "../../env.js";
import { decideListing } from "./listings.js";
...
async function tellThemWhatIsWrong(documentId: string, notes?: string): Promise<void> {
  ...
  try {
    await sendVerificationChangesRequested({
      ...
      actionUrl: `${env.STUDIO_APP_URL}/verification`,
    });
  } catch {
...
/**
 * One click for the whole application: the vendor, their two ID documents,
 * and every listing they submitted with it — approved together, because they
 * were reviewed together on this one page. Nothing from this application is
 * left sitting in the separate documents/listings queues afterward.
 *
 * A listing added later from Business Studio still goes through the ordinary
 * one-at-a-time review in /admin/listings — this only covers what arrived
 * bundled with the application, still pending_approval at the moment of
 * approval.
 */
export async function approveProviderAndListings(
  actorId: string,
  providerId: string,
): Promise<{ warning?: string }> {
  const db = adminDb();
  ...
  for (const doc of documents ?? []) {
    if (doc.status !== "approved") await decideDocument(actorId, doc.id, true);
  }
  ...
  for (const listing of listings ?? []) {
    await decideListing(actorId, listing.id, "approved");
  }

  return approveProvider(actorId, providerId);
}
